...
/* $Id: //dvt/mti/rel/6.5b/src/vsim/svdpi.h#1 $ */
...
/*
 * Vector Val utility functions.
 *
 * The value of the complete vector is extracted from the
 * source representation and written into the destination.
 *
 * s=source, d=destination, w=width
 */
XXTERN void svCopyVecValBit(svBitVecVal* d, const svBitVecVal* s, int w);
XXTERN void svCopyVecValLogic(svLogicVecVal* d, const svLogicVecVal* s, int w);
