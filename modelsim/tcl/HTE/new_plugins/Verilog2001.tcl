#HTEParser#verilog2001#LexVerilog2001#Verilog files#Verilog 2001 Plugin#v|vlog#
# =============================================================================
#
...
#   HTE::registerInitScript [namespace current]::init
   ...
   if [info exists HTE::Configuration(LexParser.[namespace tail [namespace current]],Browser.ShowBlocks)] {
      ...
      set HTE::Configuration(LexParser.[namespace tail [namespace current]],Browser.ShowBlocks) \
         {always function gateInstance generate genvar initial instance instanceExternal localparam module parameter parameterMap portInOut portIn portMap portOut signal task seqblk parblk}
...
#  putLog "function: start parsing at $start"
   ...
   # automatic or signed
   set asStart $scanStartIndex
   set auto_or_signed [[namespace current]::getExactLexeme $id $w $wDirect $op scanStartIndex {automatic|signed}]
   if { $auto_or_signed!={} } {
      if { $caller=={highlight} } { highlightKeyword $w $auto_or_signed $asStart $scanStartIndex }
      set asStart $scanStartIndex
      set auto_or_signed [[namespace current]::getExactLexeme $id $w $wDirect $op scanStartIndex {automatic|signed}]
      if {  $auto_or_signed!={} && $caller=={highlight} } {
         highlightKeyword $w $auto_or_signed $asStart $scanStartIndex
      }
   }
...
#  putLog "function: $scanStartIndex $functionName at $functionNameIndex"
   ...
   if { $openParen=={(} } {
      while 1 {
         ...
         set portDir [[namespace current]::getExactLexeme $id $w $wDirect $op scanStartIndex {input}]
         ...
         set portType [[namespace current]::getExactLexeme \
               $id $w $wDirect $op scanStartIndex {reg|signed|time|real|realtime|integer}]
         if { $portType!={} } {
            if { $caller=={highlight} } { highlightKeyword $w $portType $typeStart $scanStartIndex }
            if { $portType=={reg} || $portType=={signed} } {
               set typeStart $scanStartIndex
               set anotherType [[namespace current]::getExactLexeme $id $w $wDirect $op scanStartIndex {reg|signed}]
   ...
   if { $hash!={} } {
      ...
      while { [$w compare $scanStartIndex < $closeIndex] } {
         ...
         set paramType [[namespace current]::getExactLexeme $id $w $wDirect $op scanStartIndex \
               {signed|integer|realtime|real|time}]
         ...
         while 1 {
            ...
            if { $parameterIdentifier!={} && $caller=={highlight} } {
               $w tag add reg $parameterIdentifierIndex $scanStartIndex
            }

            set delimiter [[namespace current]::getLexeme $id $w $wDirect $op scanStartIndex {,|\)}]
            #putLog "mpl: delimiter=$delimiter at $scanStartIndex"
   ...
   if { $openParen=={(} } {
      ...
      while { [$w compare $scanStartIndex < $closeIndex] } {
         ...
         set portType [[namespace current]::getExactLexeme $id $w $wDirect $op scanStartIndex \
               {reg|wire|wand|wor|tri|triand|trior|integer|time}]
         if { $portType!={} && $caller=={highlight} } {
            highlightKeyword $w $portType $typeStart $scanStartIndex
         }

         set signedStart $scanStartIndex
         set signed [[namespace current]::getExactLexeme $id $w $wDirect $op scanStartIndex {signed}]
         if { $signed!={} && $caller=={highlight} } {
            highlightKeyword $w $signed $signedStart $scanStartIndex
         }
...
#         set closeParen [[namespace current]::getExactLexeme $id $w $wDirect $op scanStartIndex {\)}]
#         if { $closeParen=={)} } break
#         break
      }
   }
...
#  reg_declaration :
   if { $keyword=={reg} } {
      ...
      set signed [[namespace current]::getExactLexeme $id $w $wDirect $op parseIndex {signed}]
      if { $signed!={} && $caller=={highlight} } {
         highlightKeyword $w $signed $startIndex $parseIndex
      }
   }
...
#  putLog "task: start parsing at $start"
   ...
   # automatic ?
   set aStart $scanStartIndex
   set auto [[namespace current]::getExactLexeme $id $w $wDirect $op scanStartIndex {automatic}]
   if { $auto!={} && $caller=={highlight} } {
      $w tag add keyWord $aStart $scanStartIndex
   }
...
#  set endIndex   [$wDirect index "$end.0 + 1 line linestart"]
   ...
   ##set decList {portIn portInOut portOut signal parameter localparam genvar wire reg}
