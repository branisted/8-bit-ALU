...
proc ::tk_dialog {w title text bitmap default args} {
    ...
    set i 0
    foreach but $args {
	button $w.button$i -text $but -command [list set ::tk::Priv(button-$w) $i]
	if {$i == $default} {
    ...
    bind $w <Destroy> [list set ::tk::Priv(button-$w) -1]
    ...
    vwait ::tk::Priv(button-$w)
    catch {focus $oldFocus}
    catch {
	# It's possible that the window has already been destroyed,
	# hence this "catch".  Delete the Destroy handler so that
	# Priv(button-$w) doesn't get reset by it.
