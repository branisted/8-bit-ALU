...
proc ::ini::open {ini {mode r+}} {
    ...
    namespace eval ::ini::$fh {
        ...
        variable solo;     array set solo     {} ;# keys that have no assigned values
    ...
    namespace eval ::ini::$fh {
        ...
        foreach sec [array names sections] {
            ...
            foreach key [lsort -dictionary [array names data [::ini::_globescape $sec]\000*]] {
               ...
               if {![info exists solo($sec\000$key)]} {
                  puts $channel "$key=$data($sec\000$key)"
               } else {
                  puts $channel "$key" ;# this is a key that has no value assigned to it
               }
            }
            puts $channel ""
        }
    ...
    namespace eval ::ini::$fh {
        ...
        foreach line [split [read $channel] "\n"] {
            if { [string match "$char*" $line] } {
               ...
               ::set data($cur\000$line) "" ;# add an entry for the key
               ::set solo($cur\000$line) "" ;# identify it as not having a value
            }
        }
        unset char cur com
...
proc ::ini::delete {fh sec {key {}}} {
    ...
    if { $key == "" } {
        ...
        array unset ::ini::${fh}::solo [_globescape $sec]\000*
        array unset ::ini::${fh}::sections [_globescape $sec]
    }
    catch {
       unset ::ini::${fh}::data($sec\000$key)
       unset ::ini::${fh}::solo($sec\000$key)
    }
}
